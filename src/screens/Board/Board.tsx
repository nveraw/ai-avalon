...
const Board = ({
  ...
  const questSizes = QUEST_SIZES[playerNames.length] ?? QUEST_SIZES[7];
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-6">
      ...
      <CardBox className="quest-track mb-4">
        ...
        <div className="flex justify-between items-end">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="text-center">
              ...
              <div className="text-xs text-indigo-500 mt-1.5 font-serif">
                {questSizes[i]}P
              </div>
            </div>
          ))}
        ...
        <div className="flex justify-center gap-10 mt-5">
          <div className="text-center">
            <div className="text-3xl text-emerald-400 cinzel">
              {questResults.filter((q) => q === "success").length}
            </div>
            <div className="text-xs text-emerald-400 tracking-widest">
              VICTORIES
            </div>
          </div>
          <div className="w-px bg-indigo-950" />
          <div className="text-center">
            <div className="text-3xl text-red-400 cinzel">
              {questResults.filter((q) => q === "fail").length}
            </div>
      ...
      <div
        className="rejection-tracker bg-slate-950/80 border border-indigo-800 rounded-xl px-5 py-3.5 mb-4
        flex items-center gap-4"
      >
        <div className="text-xs text-purple-400 tracking-widest font-serif whitespace-nowrap">
          REJECTIONS
        </div>
      ...
      <CardBox className="playerNames mb-5">
        <SectionLabel className="text-purple-400">
          KNIGHTS OF THE REALM
        </SectionLabel>
        <div className="flex flex-wrap gap-5 justify-center">
          {playerNames.map((name) => (
            <PlayerToken key={name} name={name} isLeader={name === leader} />
          ))}
        </div>
      </CardBox>
